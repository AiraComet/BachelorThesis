...
As shown in figure \ref{fig:lift}, this can be done by lifting up the state to a new node that has all the components as children, that are affected by the update.
\cite{reactjsGS}
...
The ability to easily introduce new features and the scalability of React makes it a remarkable choice for this task.

Having an established hierarchy for every group ensures an intuitive navigation through the MathHub library in order to create an accessible user experience.
...
As of right now the clear design introduced in this paper proved to be adequate.
